...
@pytest.mark.asyncio
async def test_get_available_model_instances_async(
    httpx_mock: HTTPXMock,
    mock_client: AsyncClient,
    model_instances_response: dict[str, Any],
) -> None:
    """Test getting available model instances (async client).
...
def test_get_available_model_instances_sync(
    httpx_mock: HTTPXMock,
    mock_client: Client,
    model_instances_response: dict[str, Any],
) -> None:
    """Test getting available model instances (sync client).
    ...
    instances = mock_client.llm.get_available_model_instances()
    assert instances[0].id == 1
    assert instances[0].policy_group_id == 1
    assert instances[0].trainer_thread_id is None
    assert instances[0].user_id is None
...
async def test_get_model_instance_by_name_async(
    httpx_mock: HTTPXMock,
    mock_client: AsyncClient,
    ...
    """Test getting a model instance by name (async client).
...
def test_get_model_instance_by_name_sync(
    httpx_mock: HTTPXMock,
    mock_client: Client,
    model_instance_name: str,
    model_instances_response: dict[str, Any],
    ...
    """Test getting a model instance by name (sync client).
    ...
    if should_exist:
        instance = mock_client.llm.get_model_instance(model_instance_name)
        assert instance.id == 1
        assert instance.policy_group_id == 1
        assert instance.name == "GPT3.5"
        assert instance.created_at == datetime(
        ...
        with pytest.raises(
            ...
            mock_client.llm.get_model_instance(model_instance_name)
