...
internally manage session cookies to ensure the ongoing ability to communicate with your
...
## Manually Interacting with the Session Cookie

The Liminal client object provides a `add_session_cookie_callback` method to manually
interact with session cookies as they are generated. This is useful in situations where
you want to retrieve that cookie and do something else with (store it in a database, for
example). Every time a new session cookie is generated by the client object, the
registered callbacks will be called.

Session cookie callbacks are methods that take a single `str` parameter (denoting the
session cookie itself). These callbacks are not expected to return any value. Lastly,
when calling `add_session_cookie_callback`, the returned method allows you to cancel the
...
async def main() -> None:
    ...
    def print_session_cookie(session_cookie: str) -> None:
        """Do something with the session cookie."""
        print(f"Session cookie: {session_cookie}")

    # Register the refresh token callback:
    remove_callback = liminal.add_session_cookie_callback(print_session_cookie)
...
## Creating a Liminal Client from a Stored Session Cookie

Assuming you have a stored session cookie (collected from the callback process shown
above), it is simple to create a new Limina client using that cookie:
...
async def main() -> None:
    """Run!"""
    # Create the client:
    liminal = await client.authenticate_from_session_cookie(session="my-session-cookie")
