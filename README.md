...
## Manually Interacting with the Session

The Liminal client object provides a `add_session_callback` method to manually
interact with sessions as they are generated. This is useful in situations where
you want to retrieve that token and do something else with (store it in a database, for
example). Every time a new session is generated by the client object, the
registered callbacks will be called.

Session callbacks are methods that take a single `str` parameter (denoting the
session). These callbacks are not expected to return any value. Lastly, when
calling `add_session_callback`, the returned method allows you to cancel the
...
async def main() -> None:
    ...
    def do_something_with_session(session: str) -> None:
        """Do something with the refresh token."""
        pass

    # Register the refresh token callback:
    remove_callback = liminal.add_session_callback(do_something_with_session)
...
## Creating a Liminal Client from a Stored Session

Assuming you have a stored session (collected from the callback process shown
above), it is simple to create a new Limina client using that session:
...
async def main() -> None:
    """Create the aiohttp session and run the example."""
    # Retrieve your stored session:
    session = "12345"

    # Create the client:
    liminal = await client.authenticate_from_session(session=session)
